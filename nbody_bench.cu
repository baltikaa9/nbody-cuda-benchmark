...
// Быстрый rsqrt — float использует аппаратную MUFU.RSQ, double точный
...
// Нулевой вектор для каждого типа
    ...
    typename Traits<VEC>::S dt)
{
    ...
    // EPS2 вычисляется из скалярного типа — float для float3/float4,
    // double для double3/double4. Нет неявных даункастов.
    ...
    for (int i = 0; i < N; i++) {
        ...
        S inv   = my_rsqrt(dist2);   // float → rsqrtf (1 инструкция)
        S s     = inv * inv * inv;   // double → rsqrt (аппаратный)

        f.x += dx * s;
        f.y += dy * s;
        f.z += dz * s;
        // .w не участвует в физике — сравниваем чистую пропускную способность памяти
    }
...
// Возвращает true если ядро запустилось без ошибок
                  ...
                  cudaEvent_t evStart, cudaEvent_t evStop)
{
    ...
    // calloc обнуляет .w для float4/double4
    ...
    // ── Прогрев: выявляем too-many-registers до старта таймера ──────
    ...
    if (ok) {
        ...
        if (kernelOk(label)) {
            ...
            // ~20 FP-операций на пару (3 sub, 6 mul, 3 add, rsqrt, 2 mul, 3 mul, 3 add)
...
int main() {
    ...
    for (int ni = 0; ni < 5; ni++) {
        ...
        for (int bi = 0; bi < 6; bi++) {
            int BS = BS_list[bi];
            runBenchmark<float3> (N, BS, iters, evStart, evStop);
            runBenchmark<float4> (N, BS, iters, evStart, evStop);
            runBenchmark<double3>(N, BS, iters, evStart, evStop);
            runBenchmark<double4>(N, BS, iters, evStart, evStop);
        }
        puts("");
    }
